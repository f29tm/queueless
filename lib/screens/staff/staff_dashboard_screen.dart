      ...
      child: Scaffold(
        ...
        body: StreamBuilder<QuerySnapshot>(
          ...
          builder: (context, snapshot) {
            ...
            // Staff streams double as the fan-out trigger — patients can't
            // read the lane, so a staff session must write their positions.
            QueuePositionFanout.autoSync(
              snapshot.data!.docs,
              NotificationService(),
            );

            // Canonical lane order (Emergency → Urgent → Non-Urgent →
            // Manual), matching the nurse dashboard and patient positions.
            final docs = snapshot.data!.docs.toList()
              ..sort(
                (a, b) => LaneOrder.compare(
...
class _VitalsSheetState extends State<_VitalsSheet> {
  ...
  Stage1Request _stage1FromDoc() {
    final inputs = _data['stage1Inputs'] as Map<String, dynamic>? ?? {};
    return Stage1Request(
      ...
      patientsPerHour: (inputs['patients_per_hour'] as num?)?.toInt() ?? 8,
    );
  }
  ...
  Future<void> _finalizeTriage() async {
    ...
    try {
      ...
      final finalTriage = _predictionToTriage(_finalPriority ?? aiPrediction);
          ...
          children: [
            ...
            Form(
              ...
              child: Column(
                children: [
                  _vitalsRow(
                    left: _vitalField(_sbpController, "SBP (mmHg)", "e.g. 120"),
                    right: _vitalField(_dbpController, "DBP (mmHg)", "e.g. 80"),
                  ),
                  ...
                  DropdownButtonFormField<int>(
                    ...
                    items: const [
                      ...
                      DropdownMenuItem(value: 2, child: Text("2 — Emergency")),
                      DropdownMenuItem(value: 3, child: Text("3 — Urgent")),
                      DropdownMenuItem(
                        value: 4,
                        child: Text("4 — Semi-urgent"),
                      ),
        ...
        children: [
          ...
          if (changed) ...[
            ...
            _warningBanner(
              ...
              color: Colors.orange,
              text: "Result changed: $stage1Prediction → ${result.prediction}",
            ),
          ],
          if (result.deferred) ...[
            const SizedBox(height: 8),
            _warningBanner(
              ...
              color: Colors.red,
              text: "Low confidence — verify with clinical judgement",
            ),
          ],
        ],
      ),
