...
void main() {
  ...
  group('extraction request contract', () {
    ...
    test(
      'is deterministic: temperature 0 and JSON response mime type',
      () async {
    ...
    test(
      'schema contains exactly the six addendum fields and nothing else',
      () async {
        await run('I fell off a ladder');
        final schema =
    ...
    test(
      'schema contains NO severity/urgency/acuity/triage-level field',
      () async {
        await run('I fell off a ladder');
  ...
  group('extraction output', () {
    test('parses all fields from a fully-determined description', () async {
      ...
      final result = await SymptomExtractionService(
        client: client,
    ...
    test(
      'leaves undetermined fields null rather than guessed (FR-NLP-04)',
      () async {
        final client = MockClient(
          (_) async => _geminiJsonResponse({
    ...
    test(
      'toJson holds exactly the six schema keys and no severity field',
      () async {
        final client = MockClient(
          (_) async => _geminiJsonResponse({
  ...
  group('extraction failure falls back to manual entry', () {
    ...
    test(
      'returns null for a blank transcript without calling the API',
      () async {
        var called = false;
